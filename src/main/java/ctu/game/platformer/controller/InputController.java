...
public class InputController implements GLFWKeyCallbackI {
    ...
    // Key remapping for handling keyboard layout issues
    private static final Map<Integer, Integer> KEY_MAPPINGS = new HashMap<>();
    static {
        // Map DELETE key to D key (observed issue)
    ...
    public void invoke(long window, int key, int scancode, int action, int mods) {
        ...
        // Map problematic keys (like DELETE -> D)
        ...
        if (key >= 0 && key <= GLFW.GLFW_KEY_LAST) {
            // Handle both PRESS and REPEAT as "key is down"
        ...
        // Notify listeners about the key event
    ...
    private static final int[] MOVEMENT_KEYS = {KEY_W, KEY_A, KEY_S, KEY_D};
